// NotePadKiller.cpp : Este archivo contiene la función "main". La ejecución del programa comienza y termina ahí.
//
...
char sNombreServicio[] = "NotePadKiller";
LPWSTR nombreServicio;
...
void main(void)
{
	...
	wchar_t wtext[20];
	mbstowcs(wtext, sNombreServicio, strlen(sNombreServicio) + 1);//Plus null
	nombreServicio = wtext;
...
DWORD WINAPI corre(LPVOID lpParameter) {
	...
	while (true) {
		...
		for (i = 0; i < cantidadProcesos; i++) {
			...
			LPCTSTR lpszTest2 = TEXT("Hola");
			if (hProcess) {
				if ((*pEnumProcessModules)(hProcess, &hMod, sizeof(hMod), &cbNeeded)) {
					(*pGetModuleBaseName)(hProcess, hMod, (LPWSTR)szProcessName, sizeof(szProcessName) / sizeof(TCHAR));
...
void WINAPI ServiceMain(DWORD dwArgc, LPTSTR *lpszArgv) {
	...
	hCodigoDll = LoadLibrary(TEXT("psapi.dll"));
	if (hCodigoDll) {
